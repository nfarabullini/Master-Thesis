from common_funs import compute_df_query, compute_files_ls, dtw_horizontal, dtw_ed
...
pd.DataFrame(dendro_arr_complete).to_csv("DTW_sim_arr_horizontal_filtered_csv")
clustering = AgglomerativeClustering(n_clusters = 9, affinity = 'precomputed', linkage = 'average').fit(dendro_arr_complete)
file1 = open("DTW_sim_arr_horizontal_filtered.txt","w")
...
plt.savefig('./Dendrograms/DTW_dendro_horizontal_filtered.png', format='png', bbox_inches='tight')
