from common_funs import compute_df_query, compute_files_ls, vector_vertical
...
for index_query in range(0, 52):
    ...
    for g in range(index_query + 1, 52):
        ...
        dtw_sim = vector_vertical(newDF, newDF_query, n_frames)
...
pd.DataFrame(dendro_arr_complete).to_csv("Vector_vertical_sim_arr_filtered_csv")
clustering = AgglomerativeClustering(n_clusters = 9, affinity = 'precomputed', linkage = 'average').fit(dendro_arr_complete)
file1 = open("Vector_vertical_arr_filtered.txt","w")
...
plt.savefig('./Dendrograms/Vector_vertical_dendro_filtered.png', format='png', bbox_inches='tight')
